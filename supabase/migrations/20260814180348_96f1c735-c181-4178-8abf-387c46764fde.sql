...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE t.type = 'cashin' AND t.status IN ('PAID_OUT', 'PAID', 'COMPLETED', 'RECEIVED', 'APPROVED')),
  ...
  SELECT COUNT(*), COALESCE(SUM(COALESCE(t.liquid_amount, t.amount)), 0)
    ...
    FROM public.transactions t
   WHERE t.user_id = _owner_id
     AND t.type = 'cashin' AND t.status IN ('PAID_OUT', 'PAID', 'COMPLETED', 'RECEIVED', 'APPROVED')
...
BEGIN
  ...
  RETURN GREATEST(_liquido - _saques_pagos, 0);
END;
$$;
...
BEGIN
  ...
  SELECT COALESCE(SUM(f.valor), 0)
   ...
   WHERE f.user_id = _user_id
     AND f.tipo = 'gastos'
     AND (_start_date IS NULL OR f.data >= _start_date)
     AND (_end_date IS NULL OR f.data <= _end_date);
  ...
  SELECT
    ...
    _liquido::numeric AS faturamento_liquido,
    (_liquido - _anuncios - (_qtd * _taxa_bot_por_venda) - (_meses * _imposto_mensal))::numeric AS lucro,
    (_taxa_gateway + (_qtd * _taxa_bot_por_venda))::numeric AS total_taxas,
    (_meses * _imposto_mensal)::numeric AS total_imposto,
