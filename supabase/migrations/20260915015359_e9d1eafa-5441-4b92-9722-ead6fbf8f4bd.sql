CREATE OR REPLACE FUNCTION public.period_start_utc(_periodo text, _ref timestamp with time zone DEFAULT now())
...
AS $function$
  SELECT CASE _periodo
    ...
    WHEN 'ontem' THEN
      ((date_trunc('day', ((_ref AT TIME ZONE 'UTC') - interval '3 hours')::timestamp) + interval '3 hours') AT TIME ZONE 'UTC') - interval '1 day'
    WHEN '7d' THEN
      ((date_trunc('day', ((_ref AT TIME ZONE 'UTC') - interval '3 hours')::timestamp) + interval '3 hours') AT TIME ZONE 'UTC') - interval '6 days'
    WHEN '30d' THEN
      ((date_trunc('day', ((_ref AT TIME ZONE 'UTC') - interval '3 hours')::timestamp) + interval '3 hours') AT TIME ZONE 'UTC') - interval '29 days'
    WHEN 'mes' THEN
      ((date_trunc('month', ((_ref AT TIME ZONE 'UTC') - interval '3 hours')::timestamp) + interval '3 hours') AT TIME ZONE 'UTC')
    WHEN '2m' THEN
      ((date_trunc('month', ((_ref AT TIME ZONE 'UTC') - interval '3 hours')::timestamp) + interval '3 hours') AT TIME ZONE 'UTC') - interval '1 month'
    ELSE '1970-01-01'::timestamptz
  END;
$function$;

CREATE OR REPLACE FUNCTION public.get_dashboard_metrics(_user_id uuid, _periodo text DEFAULT 'mes'::text)
RETURNS TABLE(total_bruto numeric, total_liquido numeric, taxa_gateway numeric, taxa_bot numeric, total_taxas numeric, total_imposto numeric, total_anuncios numeric, total_repasses_aprovados numeric, total_reembolsos numeric, qtd_vendas bigint, qtd_reembolsos bigint, lucro_total numeric, roi numeric, taxa_media_pct numeric)
LANGUAGE plpgsql
...
BEGIN
  ...
  _end_utc := CASE _periodo
    ...
    WHEN 'ontem' THEN _start_utc + interval '1 day'
    ELSE now()
  END;
